...
import { Play, Square, RotateCcw, Activity, Zap, TrendingUp, AlertCircle, Lock } from 'lucide-react';
...
export const TrainingDashboard: React.FC<TrainingDashboardProps> = ({
  ...
  const isTrainingEnabled = useFeature('training-dashboard');
  ...
  // Show disabled state if training dashboard is not enabled
  if (!isTrainingEnabled) {
    return (
      <div className="bg-gray-900 rounded-lg border border-gray-800 p-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-3">
            <Activity className="text-gray-500" size={28} />
            <h3 className="text-xl font-bold text-gray-400">Neural Network Training</h3>
        ...
        <div className="text-center text-gray-500 py-8">
          ...
          <p className="text-lg font-medium mb-2">Training Dashboard Disabled</p>
          <p className="text-sm">This feature is currently disabled. Enable it in the feature flags to access AI training controls.</p>
        </div>
      </div>
    );
  }
