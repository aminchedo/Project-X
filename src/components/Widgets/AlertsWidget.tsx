...
const AlertsWidget: React.FC = () => {
  ...
  const fetchAlerts = async () => {
    try {
      if (api && api.trading && api.trading.getAlerts) {
        ...
        setAlerts([
          { id: '1', type: 'success', message: 'Portfolio performance updated', timestamp: new Date().toISOString() },
          { id: '2', type: 'warning', message: 'High volatility detected in BTCUSDT', timestamp: new Date(Date.now() - 300000).toISOString() },
          { id: '3', type: 'info', message: 'New trading signal available', timestamp: new Date(Date.now() - 600000).toISOString() }
        ]);
      }
    } catch (err) {
