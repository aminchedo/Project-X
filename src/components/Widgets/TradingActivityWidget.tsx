...
const TradingActivityWidget: React.FC = () => {
  ...
  const fetchRecentTrades = async () => {
    try {
      if (api && api.trading && api.trading.getRecentTrades) {
        const response = await api.trading.getRecentTrades({ limit: 5 });
        setRecentTrades(response || []);
      } else {
        // Use mock data if API is not available
        console.warn('API not available, using mock trading activity data');
        setRecentTrades([
          { id: '1', symbol: 'BTCUSDT', type: 'BUY', quantity: 0.5, price: 50123.45, timestamp: new Date().toISOString() },
          { id: '2', symbol: 'ETHUSDT', type: 'SELL', quantity: 2.0, price: 3245.67, timestamp: new Date(Date.now() - 300000).toISOString() },
          { id: '3', symbol: 'BNBUSDT', type: 'BUY', quantity: 10, price: 456.78, timestamp: new Date(Date.now() - 600000).toISOString() }
        ]);
      }
    } catch (err) {
