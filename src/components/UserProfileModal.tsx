  ...
  const fetchProfile = async () => {
    try {
      if (api && api.trading && api.trading.getUserProfile) {
        ...
        // API not available - set profile to null instead of using fake data
        console.warn('User profile API not available');
        setProfile(null);
      }
    } catch (err) {
            ...
            <div className="p-6 space-y-6">
              {profile ? (
                ...
                <div className="py-12 text-center">
                  ...
                  <p className="text-slate-400 mb-2">No profile data available</p>
                  <p className="text-slate-500 text-sm">Check backend connection or configure user profile API</p>
                </div>
              )}
            </div>
          </motion.div>
